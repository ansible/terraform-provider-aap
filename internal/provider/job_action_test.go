...
func getAllJobResults(results []interface{}, url string) ([]interface{}, error) {
	body, err := testMethodResource(http.MethodGet, url)
	if err != nil {
		return nil, err
	}
	...
	if data["next"] != nil {
		...
		return getAllJobResults(results, next)
	}
	return results, nil
}
...
func testAccJobLaunched(_ *testing.T, timestamp string) resource.TestCheckFunc {
	return func(_ *terraform.State) error {
		results, err := getAllJobResults(nil, fmt.Sprintf("api/controller/v2/jobs?unified_job_template=%s", os.Getenv("AAP_TEST_JOB_TEMPLATE_ID")))
		if err != nil {
			return err
		}
